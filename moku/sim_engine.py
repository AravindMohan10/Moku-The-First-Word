...
def _llm_policy(c: Creature, state: WorldState, r: random.Random) -> tuple[CreatureTurn, dict[str, Any]]:
    ...
    system = (
        ...
        "Glyphs and private meanings are the main story. Under scarcity you may deceive with misleading glyphs. "
        "Social bonds: faint trust lines on the map appear ONLY when you use signal, follow, or share_food "
        "with target set to another creature name from valid_creature_names (never yourself). "
        "When nearby_creatures is non-empty, use social actions sometimes (not every turn): "
        "signal + target to call or warn someone by name; "
        "follow + target to stay with a trusted ally; "
        "share_food + target when you hold food and a nearby creature is hungry (not signal for feeding). "
        "Raise trust_updates toward creatures you signal, follow, or feed; lower toward those who mislead you. "
        "Move often toward visible_food when hungry; roam instead of endless stay. "
